...
BASE_POSTERIOR_DIR="$EXPERIMENT_DIR/pretrain/sgld/cifar10/sgld_mom_0.0_preconditioner_None__lr_sch_constant_i_1e-06_f_1e-06_c_50_bi_1000___epochs_10000_wd_5.0_batchsize_80_temp_1.0__seed_$PRETRAINED_SAMPLING_SEED"
RUNWD="$HOME/bnn_hmc/"
cd $RUNWD
...
python bnn_hmc/sample_synthetic_labels.py \
    --seed=$SEED \
    --dir=$EXPERIMENT_DIR/synthdata/sgld/cifar10 \
...
SYNTHDATADIR=$(ls $EXPERIMENT_DIR/synthdata/sgld/cifar10 2>/dev/null | grep -E "^sample_from_checkpoint_.*_subset_8160_split_2_of_2__seed_$SEED$")
...
SYNTHDATAFILE=$(ls $EXPERIMENT_DIR/synthdata/sgld/cifar10/$SYNTHDATADIR 2>/dev/null | grep -E "^synth_appended_.*.npz$")
...
python bnn_hmc/run_sgmcmc.py \
    --seed=$SEED \
    --weight_decay=5. \
    --dir=$EXPERIMENT_DIR/retrain/sgld/cifar10/ \
    --dataset_name=$EXPERIMENT_DIR/synthdata/sgld/cifar10/$SYNTHDATADIR/$SYNTHDATAFILE \
    --subset_train_to=8160 \
    --model_name=resnet20_frn_swish \
    --init_step_size=1e-6 \
