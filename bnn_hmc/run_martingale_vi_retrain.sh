...
BASE_POSTERIOR_DIR="/mnt/disks/checkpoints/martingale/pretrain/vi50/cifar10/mfvi_initsigma_0.01_meaninit__opt_adam__lr_sch_i_0.0001___epochs_300_wd_5.0_batchsize_80_temp_1.0__seed_$PRETRAINED_SAMPLING_SEED"
RUNWD="$HOME/bnn_hmc/"
cd $RUNWD
...
python bnn_hmc/sample_synthetic_labels.py \
    --seed=$SEED \
    --dir=$EXPERIMENT_DIR/synthdata/vi50/cifar10 \
    --model_name=resnet20_frn_swish \
    --dataset_name=cifar10 \
    --subset_train_to=40800 \
    --sequential_training \
    --num_sequential_training_folds=2 \
    --index_sequential_training_fold=1 \
    --vi_checkpoint=$BASE_POSTERIOR_DIR/model_step_299.pt \
    --append_synthetic_dataset_to=$BASE_POSTERIOR_DIR/data_subset_20400.npz

# Find the output file produced by SGD for the second half
SYNTHDATADIR=$(ls $EXPERIMENT_DIR/synthdata/vi50/cifar10 2>/dev/null | grep -E "^sample_from_vi_.*_subset_40800_split_2_of_2__seed_$SEED$")
...
python bnn_hmc/run_sgd.py \
    --seed=$SEED \
    --weight_decay=10 \
    --dir=$EXPERIMENT_DIR/retrain/sgd_for_vi50/cifar10/ \
    --dataset_name=$EXPERIMENT_DIR/synthdata/vi50/cifar10/$SYNTHDATADIR/$SYNTHDATAFILE \
    --subset_train_to=40800 \
    --model_name=resnet20_frn_swish \
    --init_step_size=3e-7 \
    --num_epochs=500 \
...
python bnn_hmc/run_vi.py \
    --seed=$SEED \
    --weight_decay=5. \
    --dir=$EXPERIMENT_DIR/retrain/vi50/cifar10/ \
    --dataset_name=$EXPERIMENT_DIR/synthdata/vi50/cifar10/$SYNTHDATADIR/$SYNTHDATAFILE \
    --subset_train_to=40800 \
    --model_name=resnet20_frn_swish \
    --mean_init_checkpoint=$EXPERIMENT_DIR/retrain/sgd_for_vi50/cifar10/sgd_mom_0.9__lr_sch_i_3e-07___epochs_500_wd_10.0_batchsize_80_temp_1.0__seed_$SEED/model_step_499.pt \
    --init_step_size=1e-4 \
    --optimizer=Adam \
    --vi_sigma_init=0.01 \
    --temperature=1. \
